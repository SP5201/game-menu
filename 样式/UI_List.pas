...
uses
  Windows, SysUtils, Types, XCGUI, UI_Theme, UI_ScrollBar;

const
  cListColIndicator = 'col_indicator';
  ...
  cListColIndexIndicator = 0;
  cListColIndexStatus = 1;
  cListColIndexTime = 2;
  cListColIndexType = 3;
  cListColIndexFeature = 4;
  cListColIndexSummary = 5;
  cSafeLogListTemplateXml = 'Resource\Layout\SafeLogList_Item.xml';
  cListHeaderSplitLineInset = 8;
  cListHeaderTextIndicator = '';
...
var
  ...
  rcDot: TRect;
begin
  half := cStatusDotDiameter div 2;
  cx := (ARcItem.Left + ARcItem.Right) div 2;
  cy := (ARcItem.Top + ARcItem.Bottom) div 2;
  rcDot := Rect(cx - half, cy - half, cx + half, cy + half);
  if ASuccess then
    XDraw_SetBrushColor(AHDraw, UITheme_StatRamFill)
  else
    XDraw_SetBrushColor(AHDraw, UITheme_ProgressBarFill);
  XDraw_FillEllipse(AHDraw, rcDot);
end;

function StatusTextIsSuccess(const AText: PWideChar): Boolean;
begin
...
begin
  ...
  if pItem.iSubItem = cListColIndexIndicator then
  begin
    pText := XList_GetItemText(hEle, pItem.index, cListColIndexStatus);
    DrawStatusDot(hDraw, pItem.rcItem, StatusTextIsSuccess(pText));
    pbHandled^ := True;
    Exit;
  end;
