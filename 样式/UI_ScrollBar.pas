...
type
  ...
  public
    ...
    procedure ApplyDefaultStyle(MinLength: Integer = 36);
    /// <summary>快捷方法：一次性为指定宿主应用默认样式。</summary>
    class procedure ApplyDefault(Hele: Integer; MinLength: Integer = 36); static;
    /// <summary>快捷方法：一次性为指定宿主应用横向滚动条默认样式。</summary>
    class procedure ApplyDefaultH(Hele: Integer; MinLength: Integer = 36); static;
...
procedure TScrollBarUI.ApplyDefaultStyle(MinLength: Integer);
begin
  if FHost = 0 then
    Exit;

  XSView_SetScrollBarSize(FHost, 7);
...
class procedure TScrollBarUI.ApplyDefault(Hele: Integer; MinLength: Integer);
var
  SB: TScrollBarUI;
begin
  SB := TScrollBarUI.Create(Hele);
  try
    SB.ApplyDefaultStyle(MinLength);
  finally
    SB.Free;
  end;
end;

class procedure TScrollBarUI.ApplyDefaultH(Hele: Integer; MinLength: Integer);
var
  SB: TScrollBarUI;
begin
