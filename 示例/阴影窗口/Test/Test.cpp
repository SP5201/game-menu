...
public:
	...
	void Init()
	{
		...
		XObj_SetTypeEx(XBtn_Create(150, 80, 200, 25, L"炫彩界面库-窗口阴影演示(关闭)", m_hWindow),button_type_close);
		...
		XEle_SetTextColor(m_hBtn_Check_rightAnge, RGBA(255,255,255,255));
		...
		m_hRichEditColor=XEditColor_Create(left+10,top, 100,22,m_hWindow);
		XEle_SetTextColor(m_hRichEditColor, RGBA(255,255,255,255));
		...
		XEle_RegEventCPP1(m_hRichEditColor,XE_EDIT_CHANGED,&CWindowShadow::OnRichEditChange_ShadowColor);
	...
	{
		m_clrShadow=XEditColor_GetColor(hRichEditColor);
		XWnd_SetShadowInfo(m_hWindow,m_nShadowSize,m_nShadowDepth,m_nShadowAngleSize,m_bRightAngle,m_clrShadow);
		XWnd_Redraw(m_hWindow);
		return 0;
	}
	int  OnWndDrawWindow(HDRAW hDraw,BOOL *pbHandled)
	{
		*pbHandled=TRUE;
		...
		XDraw_SetBrushColor(hDraw, RGBA(250,250,250,255));
