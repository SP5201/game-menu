...
var
  ...
  GListViewContextItemIndex: Integer = -1;
  GListViewContextFilePath: string = '';
...
begin
  ...
  if (nItem = ID_LISTVIEW_MENU_OPEN) or (nItem = ID_LISTVIEW_MENU_RUN_AS_ADMIN) or
    (nItem = ID_LISTVIEW_MENU_OPEN_FOLDER) then
  begin
...
begin
  ...
  TMainWindowStat.NotifyWindowActivation(Msg, wParam);
...
function MainWindowIsSearchListContext(ASender: TListViewUI): Boolean;
begin
  if (ASender <> nil) and ASender.IsSearchMode then
    Exit(True);
  if TMainFormUI.CActiveListGroupIndex < 0 then
    Exit(True);
  Result := (TMainFormUI.CListBoxUI <> nil) and
...
var
  ...
  groupIndex: Integer;
begin
  Result := GListViewContextFilePath;
  AItemIndex := GListViewContextItemIndex;
  GListViewContextFilePath := '';
  GListViewContextItemIndex := -1;
  if (Result = '') and (AItemIndex >= 0) and (TMainFormUI.CListViewUI <> nil) and
    TMainFormUI.CListViewUI.IsSearchMode then
    Result := TMainFormUI.CListViewUI.ResolveSearchItemPath(AItemIndex);
  if Result <> '' then
    Exit;
  if TMainFormUI.CListViewUI = nil then
    Exit;
  if not TMainFormUI.CListViewUI.TryGetSelectedItem(groupIndex, AItemIndex, data) then
    Exit;
...
begin
  ...
  GListViewContextItemIndex := AItemIndex;
  GListViewContextFilePath := AFilePath;
  isSearchView := MainWindowIsSearchListContext(ASender);
...
begin
  if ASender = nil then
    Exit;
  filePath := '';
  if ASender.TryGetItem(AItemIndex, item) then
  begin
    filePath := item.FilePath;
    if filePath = '' then
      filePath := ASender.ResolveSearchItemPath(AItemIndex);
  end;
  GListViewContextItemIndex := AItemIndex;
  GListViewContextFilePath := filePath;
