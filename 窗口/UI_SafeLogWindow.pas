...
begin
  ...
  iRow := XList_AddItemText(CListEle, PWideChar(statusText));
  if iRow < 0 then
    Exit;
...
begin
  hOwner := 0;
  if (CInstance <> nil) and CInstance.IsHWINDOW then
    hOwner := XWnd_GetHWND(CInstance.Handle);
...
begin
  ...
  begin
    ...
    XEdit_SetDefaultTextColor(hEdit, UITheme_TextPlaceholder);
...
begin
  ...
  begin
    if CInstance.IsHWINDOW then
      XWnd_ShowWindow(CInstance.Handle, SW_SHOW);
    Exit;
  end;
