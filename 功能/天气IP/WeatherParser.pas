...
begin
  ...
  Result.Kind := hkText;
  Result.SaveToFile := '';
  Result.Options := TNetHttpWorker.DefaultOptions;
  Result.UserData := 0;
end;
...
begin
  ...
  case AWmoCode of
    ...
    2: ApplyDayNightIcon(Result, 101, 153, AIsDay);
    3: ApplyDayNightIcon(Result, 104, 154, AIsDay);
    45, 48: Result := 501;
    51, 53, 55: Result := 305;
    56, 57: Result := 313;
    61: Result := 305;
    ...
    75: Result := 402;
    77: Result := 400;
    80, 81, 82: ApplyDayNightIcon(Result, 300, 350, AIsDay);
...
begin
  ...
  if HasSubstr('雾') or HasSubstr('浓雾') then Exit(501);
  if HasSubstr('薄雾') then Exit(500);
  if HasSubstr('霾') then Exit(502);
  if HasSubstr('阴') then
    ApplyDayNightIcon(Result, 104, 154, AIsDay)
...
begin
  ...
  if msg = 'Only https URL supported' then
    Exit('仅支持 HTTPS 地址');
  if (Pos(' Connect ', msg) > 0) or (Pos('WinHttpOpen', msg) > 0) or
     (Pos('OpenRequest', msg) > 0) or (Pos('SendRequest', msg) > 0) then
    Exit('无法连接服务器');
