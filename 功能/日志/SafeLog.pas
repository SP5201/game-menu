...
uses
  SysUtils, Classes, SyncObjs, Windows, XCGUI, AppPaths;
...
procedure SafeLogRecord(const ATypeText, AFeature, ASummary: string; ASuccess: Boolean; const AParsedLine: string = '');
...
begin
  ...
  try
    ...
    sl.Add('界面库版本: ' + GetDllFileVersionText(AppBinDllPath('XCGUI.dll')));
    sqliteVer := GetSqlite3VersionText;
    if sqliteVer <> '' then
      sl.Add('数据库版本: ' + sqliteVer)
    else
      sl.Add('数据库版本: ' + GetDllFileVersionText(AppBinDllPath('Sqlite3.dll')));
    Result := Trim(sl.Text);
  finally
    sl.Free;
  end;
...
function SafeLogWindowActive: Boolean;
begin
  Result := SafeLogLiveWindowHandle <> 0;
end;

procedure SafeLogPostAppend(AEntryId: Integer);
begin
  if SafeLogWindowActive then
...
begin
  ...
  entry.ParsedLine := Trim(AParsedSingleLine);
...
begin
  ...
  entry.ElapsedText := '-';
  entry.ParsedLine := Trim(AParsedLine);
...
begin
  if not GStartupActive then
    Exit;
  line := Trim(AFeature) + '：' + Trim(ADetail);
...
begin
  ...
  if SameText(AEntry.TypeText, '搜索') then
  begin
    if AEntry.Success then
      Result := '获取结果：成功'
    else
  ...
  if SameText(AEntry.TypeText, '索引') or SameText(AEntry.TypeText, '调试') then
  begin
    Result := AEntry.SummaryText;
    Result := Result + sLineBreak + '时间：' + AEntry.FetchTimeText;
    if Trim(AEntry.ParsedLine) <> '' then
      Result := Result + sLineBreak + AEntry.ParsedLine;
    Exit;
