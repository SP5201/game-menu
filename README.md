...
flowchart TB
  ...
  subgraph Lib["基础层"]
    L["库/ — XCGUI · SQLite3 · WinHTTP · ZXing …"]
  end
  W --> S --> F --> L
...
3. **主题** — 语义色在 `UI_Theme.pas`；窗口 `ApplyDefaultStyles` 仅做 CPU/RAM/GPU 分色。
...
```mermaid
flowchart TD
  Q[悬停 / 详情查询] --> R{NwInfo 可用?}
  R -->|是| N[nwinfox86.exe<br/>解析 YAML]
  R -->|否| W[Win32 API 回退]
  N --> C[静态结构 + TTL 缓存]
  W --> C
  C --> P[InfoListPopup 展示]

  classDef query fill:#e1f5fe,stroke:#0288d1
  classDef ok fill:#e8f5e9,stroke:#43a047
  classDef fallback fill:#fff3e0,stroke:#fb8c00
  class Q query
  class N ok
  class W fallback
```

1. `NwInfoRunner` 调用 `Resource\NwInfo\nwinfox86.exe`（libnw 无独立导出 API）。
2. 解析 stdout 填入结构体；传感器字段按 TTL 缓存。
3. 缺字段时用 `GlobalMemoryStatusEx`、`EnumDisplayDevicesW`、`GetAdaptersAddresses` 等补全。

**使用率**（高频）与 **详情 tooltip**（低频）分离：`HardwareMonitor` 刷新百分比，首次悬停详情可能略慢。

### HTTP 与配置

- 所有 WinHTTP 集中在 `NetHttpWorker` **单工作线程**。
...
| `功能/硬件/` | 状态栏采样、`nwinfox86` 子进程 |
| `功能/硬件/CPU/` · `Mem/` · `GPU/` · `Net/` | 各硬件详情与网速 |
| `功能/天气IP/` | WinHTTP、外网 IP、天气解析 |
