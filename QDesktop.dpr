...
begin
  ...
  try
    SafeLogStartupBegin;
    SafeLogStartupAppendRuntimeEnv;
    configExisted := FileExists(TAppConfig.ConfigFilePath);
    TAppConfig.Load;
    if configExisted then
      SafeLogStartupAppendStep(string('配置'), string('加载配置文件'), True)
    else
      SafeLogStartupAppendStep(string('配置'), string('已创建默认配置 QDesktop.ini'), True);
    TAppSettings.SyncRunWithWindowsRegistry(TAppConfig.IsRunWithWindows);
    if not TAppSettings.InitXCGUI then
    begin
      SafeLogStartupAppendStep(string('资源'), string('XCGUI 初始化失败'), False);
      SafeLogStartupCommit;
      Exit;
    end;
    if XC_IsEnableD2D then
      SafeLogStartupAppendStep(string('资源'), string('加载程序界面引擎 D2D模式'), True)
    else
      SafeLogStartupAppendStep(string('资源'), string('加载程序界面引擎 GDI模式'), True);
